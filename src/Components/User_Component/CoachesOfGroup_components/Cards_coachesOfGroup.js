...
import { useFetch, isSet, EmptyPage } from '../../Common'


const Cards_coachesOfGroup = (props) => {
  const width = 320;
  ...
  return (
    <View style={styles.container}>
      {
        isSet(data.data) ?
          isSet(data.data.listCoach) ?
            data.data.listCoach.map((value, index) => {
              return (
                <Ripple key={index}
                  onPress={() => replace('CoachsCourses',
                    {
                      idCatService: value.id,
                      idUser: value.idUser,
                      goBack:props.route.name
                    })}
                  style={{ ...styles.card, width: width / 1.8, height: width / 1.5 }}>
